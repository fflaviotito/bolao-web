...
export const Container = styled.main<StyleProps>`
    display: flex;
    align-items: center;
    ...
    @media (min-width: 1024px) {
        flex-direction: ${({ $ladoCartao }) => ($ladoCartao == 'esquerdo' ? 'row-reverse' : 'row')};
        max-width: 1264px;
        margin: auto;
        padding-top: 0;
        padding-bottom: 0;
...
export const LadoMarca = styled.header<StyleProps>`
    display: none;
    ...
    @media (min-width: 1024px) {
        width: 100%;
        gap: 80px;
        align-items: ${({ $ladoCartao }) => ($ladoCartao == 'esquerdo' ? 'end' : 'start')};
        ...
        > div {
            ...
            > h1 {
                font-size: 48px;
                font-weight: bold;
                text-align: ${({ $ladoCartao }) => ($ladoCartao == 'esquerdo' ? 'right' : 'left')};
            }
